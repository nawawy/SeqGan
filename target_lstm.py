...
class TARGET_LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim, sequence_length, start_token, params):
        ...
        with tf.variable_scope('generator'):
            self.g_embeddings = tf.get_variable("g_Embedding", shape=[num_emb, self.params[0].shape[1]],
                                                initializer=tf.keras.initializers.glorot_uniform())  # tf.Variable(self.params[0])
    ...
    def create_output_unit(self, params):
        ...
        with tf.variable_scope("Output_Unit", initializer=tf.keras.initializers.glorot_uniform()) as scope:

            self.Wo = tf.get_variable("W", shape=[self.params[13].shape[0], self.num_emb])
            self.bo = tf.get_variable("b", shape=[self.num_emb])
